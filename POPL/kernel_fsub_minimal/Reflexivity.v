...
Lemma Sub_renaming: forall E1 E2 X Y A B T,
    sub (E1 ++ X ~ bind_sub T ++ E2) A B ->
    Y \notin {{X}} \u fv_tt A \u fv_tt B \u fv_tt T \u dom E1 \u dom E2 ->
    wf_env (map (subst_tb X Y) E1 ++ Y ~ bind_sub T ++ E2) ->
    sub (map (subst_tb X Y) E1 ++ Y ~ bind_sub T ++ E2) (subst_tt X Y A) (subst_tt X Y B).
Proof with auto using WF_renaming.
  intros.
  dependent induction H;simpl in *...
  -
    destruct (X0==X);subst...
  ...
  -
    ...
    +
      ...
      get_well_form...
      inversion BindsTacVal;subst.
      apply sa_trans_tvar with (U:=T)...
      assert (T = subst_tt X Y T) as Ht.
      {
    ...
    +
      ...
      dependent destruction H2...
      apply sa_trans_tvar with (U:=subst_tt X Y U)...
      analyze_binds H...
      apply In_lemmaL...
      apply binds_map_free...
      apply In_lemmaR...
      ...
      apply WF_from_binds_typ with (x:=X0) (U:=U) in H3...
      apply WF_imply_dom in H3.
      assert (U = subst_tt X Y U) as Ht.
      {
        apply subst_tt_fresh...
      ...
      apply notin_partial with (E2:=dom (E1 ++ (X, bind_sub T) :: E2))...
  -
    apply sa_all with (L:=L \u {{X}} \u {{Y}} \u dom E1 \u dom E2 \u fv_tt S \u fv_tt T1 \u fv_tt T2)...
    +
      apply WF_renaming...
    +
      intros.
      rewrite subst_tt_open_tt_var...
      rewrite subst_tt_open_tt_var...
      rewrite_env  (map (subst_tb X Y) ([(X0, bind_sub S)] ++ E1) ++ (Y, bind_sub T) :: E2).
      apply H1...
      solve_notin...
      rewrite_env ([(X0, bind_sub (subst_tt X Y S))] ++ map (subst_tb X Y) E1 ++ (Y, bind_sub T) :: E2).
      constructor...
      apply WF_renaming...
  -
    apply sa_rec with (L:=L \u {{X}} \u {{Y}} \u dom E1 \u dom E2 \u fv_tt T \u fv_tt A1 \u fv_tt A2);intros...
    +
      rewrite subst_tt_open_tt_var...
      rewrite_env  (map (subst_tb X Y) ([(X0, bind_sub typ_top)] ++ E1) ++ (Y, bind_sub T) :: E2).
      apply WF_renaming...
    ...
    +
      ...
      apply H2...
      solve_notin...
      rewrite_env ([(X0, bind_sub (subst_tt X Y typ_top))] ++ map (subst_tb X Y) E1 ++ (Y, bind_sub T) :: E2)...
Qed.
