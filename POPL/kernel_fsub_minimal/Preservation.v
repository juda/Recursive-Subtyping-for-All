...
Proof with eauto.
  ...
  -
    apply typing_fold...
    apply wf_typ_strengthening in H0...
  -
    apply typing_sub with (S:=S)...
...
Proof with auto.
  ...
  -
    ...
    apply subst_tb_wf with (Q:=bind_sub Q)...
    rewrite_env ( map (subst_tb Z P) (X~bind_sub S ++ F) ++ E).
  ...
  -
    ...
    +
      ...
      *
        ...
        rewrite subst_tt_intro with (X:=X)...
        rewrite_alist (map (subst_tb X T) nil ++ E).
        apply typing_through_subst_te with (Q:=T1)...
        apply H...
      *
        apply typing_inv_tabs with (U1:=T1) (U2:=T2) in H...
        destruct_hypos.
        pick fresh X.
        rewrite subst_te_intro with (X:=X)...
        rewrite subst_tt_intro with (X:=X)...
