Set Implicit Arguments.
Require Import Metalib.Metatheory.
Require Import Program.Equality.
Require Export Preservation.


Lemma binds_split: forall E X U,
    binds X (bind_sub U) E ->
    exists E1 E2,
      E = E1 ++ X~(bind_sub U) ++ E2.
Proof with auto.
  induction E;intros...
  analyze_binds H.
  destruct a.
  analyze_binds H.
  -
    exists nil.
    exists E...
  -
    apply IHE in BindsTac...
    destruct_hypos.
    rewrite H.
    exists ((a,b)::x).
    exists x0...
Qed.


Lemma binds_split_lb: forall E X U,
    binds X (bind_sub_lb U) E ->
    exists E1 E2,
      E = E1 ++ X~(bind_sub_lb U) ++ E2.
Proof with auto.
  induction E;intros...
  analyze_binds H.
  destruct a.
  analyze_binds H.
  -
    exists nil.
    exists E...
  -
    apply IHE in BindsTac...
    destruct_hypos.
    rewrite H.
    exists ((a,b)::x).
    exists x0...
Qed.


Inductive sub_tvar_chain : env -> env -> atom -> atom -> Prop :=
| sub_tvar_refl: forall E U X, 
    wf_env E -> binds X (bind_sub U) E ->
    sub_tvar_chain E (X ~ bind_sub U) X X
(* | sub_tvar_refl_lb: forall E U X, 
    wf_env E -> binds X (bind_sub_lb U) E ->
    sub_tvar_chain E (X ~ bind_sub_lb U) X X *)
| sub_tvar_cons: forall E1 E2 W (X1 X2 X3:atom),
    wf_env (E1 ++ X1 ~ bind_sub X2 ++ E2) ->
    sub_tvar_chain (E1 ++ X1 ~ bind_sub X2 ++ E2) W X2 X3 ->
    sub_tvar_chain 
      (E1 ++ X1 ~ bind_sub X2 ++ E2) 
      (X1 ~ bind_sub X2 ++ W) X1 X3
(* | sub_tvar_cons_right: forall E1 E2 W (X1 X2 X3:atom),
    wf_env (E1 ++ X3 ~ bind_sub_lb X2 ++ E2) ->
    sub_tvar_chain (E1 ++ X3 ~ bind_sub_lb X2 ++ E2) W X1 X2 ->
    sub_tvar_chain 
      (E1 ++ X3 ~ bind_sub_lb X2 ++ E2) 
      (X3 ~ bind_sub_lb X2 ++ W) X1 X3. *).

Inductive sub_tvar_chain_lb : env -> env  -> atom -> atom -> Prop :=
| sub_tvar_refl_lb: forall E U X, 
    wf_env E -> binds X (bind_sub_lb U) E ->
    sub_tvar_chain_lb E (X ~ bind_sub_lb U) X X
| sub_tvar_cons_lb: forall E1 E2 W (X1 X2 X3:atom),
    wf_env (E1 ++ X3 ~ bind_sub_lb X2 ++ E2) ->
    sub_tvar_chain_lb (E1 ++ X3 ~ bind_sub_lb X2 ++ E2) W X1 X2 ->
    sub_tvar_chain_lb 
      (E1 ++ X3 ~ bind_sub_lb X2 ++ E2) 
      (X3 ~ bind_sub_lb X2 ++ W) X1 X3.



Lemma sub_tvar_chain_subset: forall E W X Y,
  sub_tvar_chain E W X Y ->
  forall K V, binds K V W -> binds K V E.
Proof with auto.
  intros. induction H.
  - inversion H0. { inversion H2;subst... }
    inversion H2.
  (* - inversion H0. { inversion H2;subst... }
    inversion H2. *)
  - destruct H0...
    inversion H0...
  (* - destruct H0...
    inversion H0... *)
Qed.

Lemma sub_tvar_chain_subset_lb: forall E W X Y,
  sub_tvar_chain_lb E W X Y ->
  forall K V, binds K V W -> binds K V E.
Proof with auto.
  intros. induction H.
  - inversion H0. { inversion H2;subst... }
    inversion H2.
  - destruct H0...
    inversion H0...
Qed.

Lemma sub_tvar_chain_strengthening: forall Z U W E2 X Y,
  sub_tvar_chain (Z ~ U ++ E2) W X Y ->
  Z \notin dom W ->
  sub_tvar_chain (E2) W X Y.
Proof with auto.
  intros. dependent induction H.
  - apply sub_tvar_refl with (U:=U0)... 
    { apply wf_env_cons in H... }
    { destruct H0... inversion H0. subst.
      exfalso. apply H1. simpl... }
  (* - apply sub_tvar_refl_lb with (U:=U0)... 
    { apply wf_env_cons in H... }
    { destruct H0... inversion H0. subst.
      exfalso. apply H1. simpl... } *)
  - destruct E1.
    { inversion x;subst. exfalso.
      apply H1. simpl... }
    { inversion x;subst.
      specialize (IHsub_tvar_chain Z U (E1 ++ X1 ~ bind_sub X2 ++ E0) JMeq_refl).
      eapply sub_tvar_cons...
      rewrite <- app_comm_cons  in H.
      rewrite cons_app_one in H. apply wf_env_cons in H...
    }
  (* - destruct E1.
    { inversion x;subst. exfalso.
      apply H1. simpl... }
    { inversion x;subst.
      specialize (IHsub_tvar_chain Z U (E1 ++ X3 ~ bind_sub_lb X2 ++ E0) JMeq_refl).
      eapply sub_tvar_cons_right...
      rewrite <- app_comm_cons  in H.
      rewrite cons_app_one in H. apply wf_env_cons in H...
    }  *)
Qed.


Lemma sub_tvar_chain_strengthening_lb: forall Z U W E2 X Y,
  sub_tvar_chain_lb (Z ~ U ++ E2) W X Y ->
  Z \notin dom W ->
  sub_tvar_chain_lb (E2) W X Y.
Proof with auto.
  intros. dependent induction H.
  - apply sub_tvar_refl_lb with (U:=U0)... 
    { apply wf_env_cons in H... }
    { destruct H0... inversion H0. subst.
      exfalso. apply H1. simpl... }
  - destruct E1.
    { inversion x;subst. exfalso.
      apply H1. simpl... }
    { inversion x;subst.
      specialize (IHsub_tvar_chain_lb Z U (E1 ++ X3 ~ bind_sub_lb X2 ++ E0) JMeq_refl).
      eapply sub_tvar_cons_lb...
      rewrite <- app_comm_cons  in H.
      rewrite cons_app_one in H. apply wf_env_cons in H...
    } 
Qed.

Lemma binds_uniq: forall E X A B,
    wf_env E ->
    binds X ( A) E ->
    binds X ( B) E ->
    A = B.
Proof with auto.
  induction E;intros...
  analyze_binds H0.
  destruct a.
  analyze_binds_uniq H0.
  -
    apply uniq_from_wf_env...
  -
    analyze_binds_uniq H1...
  -
    analyze_binds_uniq H1...
    apply IHE with (X:=X)...
    dependent destruction H...
Qed.


Lemma sub_tvar_inv:forall E U (X:atom),
    sub E U X ->
    (exists Y, U = typ_fvar Y) \/ 
    (exists G, binds X (bind_sub_lb G) E /\ sub E U G) \/ 
     U = typ_bot .
     (* \/ (exists U1 U2, U = typ_and U1 U2). *)
Proof with auto.
  intros.
  dependent induction H...
  - left. exists X...
  - left. exists X0...
  - right. left. exists U...
  - destruct IHsub with (X0:=X) as [?|[?|?]]...
    * destruct H2 as [Y ?]. subst...
      apply Compatible_var_absurd in H1.
      exfalso...
    * destruct H2 as [G [? ?]].
      right. left.
      exists G...
    * subst.
      apply Compatible_bot_absurd in H1.
      exfalso...
  - destruct IHsub with (X0:=X) as [?|[?|?]]...
    * destruct H2 as [Y ?]. subst...
      apply Compatible_symm in H1.
      apply Compatible_var_absurd in H1.
      exfalso...
    * destruct H2 as [G [? ?]].
      right. left.
      exists G...
    * subst.
      apply Compatible_symm in H1.
      apply Compatible_bot_absurd in H1.
      exfalso...
Qed.

(* 
X <: {l1 ...} & {l2 ...}
  <: {l1 ...} <: Y
*)


Lemma sub_tvar_inv2: forall E U (X:atom),
    sub E X U ->
    (exists Y, U = typ_fvar Y) \/
    (exists G, binds X (bind_sub G) E /\ sub E G U) \/
    U = typ_top.
    (* \/ *)
    (* (exists U1 U2, U = typ_and U1 U2). *)
Proof with auto.
  intros.
  dependent induction H...
  - left. exists X...
  - right. left. exists U...
  - left. exists X0...
  - destruct IHsub1 with (X0:=X) as [?|[?|?]]...
    +
      destruct H2 as [Y ?]. subst...
      apply Compatible_var_absurd in H1.
      exfalso...
    +
      destruct IHsub2 with (X0:=X) as [?|[?|?]]...
      *
        destruct H3 as [Y ?]. subst...
        apply Compatible_symm in H1.
        apply Compatible_var_absurd in H1.
        exfalso...
      *
        destruct H2 as [G [? ?]].
        destruct H3 as [G' [? ?]].
        assert (wf_env E). { get_well_form... }
        pose proof binds_uniq _ _ _  H6 H2 H3.
        inversion H7;subst.
        right. left. exists G'. split...
      *
        destruct H2 as [G [? ?]]. subst.
        right. left. exists G. split...
        constructor... get_well_form...

    + 
      destruct IHsub2 with (X0:=X) as [?|[?|?]]...
      *
        destruct H3 as [Y ?]. subst...
        apply Compatible_symm in H1.
        apply Compatible_var_absurd in H1.
        exfalso...
      *
        destruct H3 as [G' [? ?]].
        assert (wf_env E). { get_well_form... }
        right. left. exists G'. subst. split...
        constructor... get_well_form...
      *
        subst. inversion H1.
        
Qed. 



(* Lemma sub_tvar_next_fvar: forall (X X0:atom) E1 E2 U,
   wf_env (E1 ++ [(X, bind_sub U)] ++ E2) ->
   sub (E1 ++ [(X, bind_sub U)] ++ E2) X X0 ->
   X <> X0 ->
   (exists Y, U = typ_fvar Y) \/
   (exists G, binds X0 (bind_sub_lb G) (E1 ++ [(X, bind_sub U)] ++ E2) ).
   (* /\ sub (E1 ++ [(X, bind_sub U)] ++ E2)  U G). *)
Proof with auto.
  intros. dependent destruction H0.
  - exfalso...
  - destruct (sub_tvar_inv H1) as [[X' Et]|G].
    + subst U0.
      assert (binds X (bind_sub U) (E1 ++ [(X, bind_sub U)] ++ E2)) by auto. 
      pose proof binds_uniq _ _ _ H H0 H3. subst U. left.
      exists X'...
    + destruct_hypos. right. exists x...
  - destruct (sub_tvar_inv2 H1) as [[X' Et]|[G|G]].
    + subst U0.
      assert (binds X (bind_sub U) (E1 ++ [(X, bind_sub U)] ++ E2)) by auto. 
      pose proof binds_uniq _ _ _ H H0 H3. subst U. left.
      exists X'...
  
  right. exists U0...
Qed. *)
  

(* TODO add mode *)


(* X1 <: U1 <: ? & ? <: U2 <: X2 

X <: 

*)

Lemma suba_sub_tvar_chain: forall E (X1 X2:atom),
    sub E X1 X2 ->
    (exists W, sub_tvar_chain E W X1 X2) \/
    (exists W, sub_tvar_chain_lb E W X1 X2) \/
    (exists U1 U2, binds X1 (bind_sub U1) E /\ binds X2 (bind_sub_lb U2) E) \/
    (sub E typ_top X2) \/ (sub E X1 typ_bot).
Proof with auto.
  intros.
  dependent induction H...
  - inversion H0;subst.
    + left. exists (X2 ~ bind_sub U).
      apply sub_tvar_refl with (U:=U)...
    + right. left. exists (X2 ~ bind_sub_lb U).
      apply sub_tvar_refl_lb with (U:=U)...
  -
    pose proof sub_regular H0. destruct_hypos.
    pose proof binds_split _ _ _ H.
    destruct H4 as [E1 [E2 H4]].
    pose proof sub_tvar_inv  H0. 
    destruct H5 as [[X' H5]|[H5|H5]].
    + subst U.
      specialize (IHsub X' X2 eq_refl eq_refl).
      destruct IHsub as [[W IHsub]|[[W IHsub]|[[U1 [U2 [IHsub1 IHsub2]]]|[IHsub|IHsub]]]].
      * subst E. left. exists (X1 ~ bind_sub X' ++ W).
        eapply sub_tvar_cons with (E2:=E2) (X2:=X')...
      * right. right. left. exists X'.
        inversion IHsub;subst.
        { exists U... }
        { exists X3... split... rewrite H7 in *... }
      * right. right. left. exists X', U2...
      * right. right. right. left...
      * right. right. right. right... apply sa_trans_tvar with (U:=X')...
    + destruct H5 as [G]. destruct_hypos.
      right. right. left. exists U, G...
    + subst U. right. right. right. right...
      apply sa_trans_tvar with (U:=typ_bot)...
    
  - pose proof sub_regular H0. destruct_hypos.
    pose proof binds_split_lb _ _ _ H.
    destruct H4 as [E1 [E2 H4]].
    pose proof sub_tvar_inv2 H0. destruct H5 as [[X' H5]|[H5|H5]].
    + subst U.
      specialize (IHsub X1 X' eq_refl eq_refl).
      destruct IHsub as [[W IHsub]|[[W IHsub]|[[U1 [U2 [IHsub1 IHsub2]]]|[IHsub|IHsub]]]].
      * right. right.
        inversion IHsub;subst.
        { left. exists U, X'... }
        { left. exists X3, X'... split... rewrite H7 in *... }
      * subst E. right. left. exists (X2 ~ bind_sub_lb X' ++ W).
        eapply sub_tvar_cons_lb with (E2:=E2) (X2:=X')...
      * right. right. left. exists U1, X'...
      * right. right. right. left. apply sub_transitivity with (Q:=X')...
        eapply sa_trans_tvar_lb with (U:=X')...
      * right. right. right. right...
    + destruct H5 as [G]. destruct_hypos.
      right. right. left. exists G, U...
    + subst U. right. right. right. left.
      apply sa_trans_tvar_lb with (U:=typ_top)...
      (* apply sub_toplike... *)
Qed.



Inductive sub_env: env -> env -> Prop :=
| sub_env_nil : forall E, sub_env E empty
| sub_env_cons_some : forall E W K V,
    sub_env E W ->
    sub_env (K ~ V ++ E) (K ~ V ++ W)
| sub_env_cons_none : forall E W K V,
    sub_env E W ->
    sub_env (K ~ V ++ E) W.

Lemma sub_env_app: forall E1 E2 W,
    sub_env E2 W ->
    sub_env (E1 ++ E2) W.
Proof. intros E1.
  induction E1;intros...
  - apply H.
  - simpl. rewrite cons_app_one.
    destruct a. apply sub_env_cons_none.
    apply IHE1. auto.
Qed.

Lemma WF_from_binds_typ_strong: 
forall (x : atom) (U : typ) (E1 E2 : env),
  wf_env (E1 ++ [(x, bind_sub U)] ++ E2) -> 
  WF E2 U.
Proof with auto.
intros.
dependent induction H.
-
  destruct E1;inversion x.
-
  destruct E1;inversion x;subst...
  specialize (IHwf_env _ _ _ _ JMeq_refl)...
-
  destruct E1;inversion x;subst...
  specialize (IHwf_env _ _ _ _ JMeq_refl)...
-
  destruct E1;inversion x;subst...
  specialize (IHwf_env _ _ _ _ JMeq_refl)...
Qed.

Lemma sub_env_clos: forall E W,
  sub_env E W ->
  forall X, X `in` dom W -> X `in` dom E.
Proof with auto.
  intros E W H.
  induction H;intros;simpl in*...
  - apply empty_iff in H. exfalso...
  - rewrite KeySetProperties.add_union_singleton in *.
    apply union_iff. apply union_iff in H0.
    destruct H0...
Qed.


Lemma sub_env_cut_head:
  forall  E2 X1 X2 T1 T2 W, 
  X1 <> X2 ->
  sub_env (X1 ~ T1 ++ E2) (X2 ~ T2 ++ W) ->
  sub_env E2 (X2 ~ T2 ++ W).
Proof with auto.
  intros.
  inversion H0;subst...
  exfalso...
Qed.

Lemma sub_env_cut_heads:
  forall E1 E2 X T W, uniq (E1 ++ X ~ T ++ E2) ->
  sub_env (E1 ++ X ~ T ++ E2) (X ~ T ++ W) ->
  sub_env E2 W.
Proof with auto.
  intros E1. induction E1...
  - intros. inversion H0;subst...
    pose proof sub_env_clos H5.
    apply uniq_cons_2 in H.
    exfalso. apply H. simpl. apply H1. simpl...
  - intros.  destruct a as [X' T'].
    simpl in H0. rewrite cons_app_one in H0.
    apply sub_env_cut_head in H0.
    2:{ apply uniq_cons_2 in H... }
    inversion H;subst.
    apply IHE1 with (X:=X) (T:=T)...
Qed.

Lemma uniq_split_eq: forall (E1 E2 E1' E2':env) (X:atom) (T T':binding),
  uniq (E1 ++ X ~ T ++ E2) ->
  E1 ++ X ~ T ++ E2 = E1' ++ X ~ T' ++ E2' ->
  E1 = E1' /\ T = T' /\ E2 = E2'.
Proof with auto.
  intros E1.
  induction E1;intros.
  - destruct E1'.
    { inversion H0... }
    destruct p. inversion H0;subst.
    apply uniq_cons_2 in H. exfalso...
    rewrite dom_app in H. simpl in H...
  - destruct E1'.
    { inversion H0. destruct a. inversion H2;subst.
      apply uniq_cons_2 in H. exfalso...
      rewrite dom_app in H. simpl in H... }
    inversion H0;subst.
    inversion H;subst.
    specialize (IHE1 _ _ _ _ _ _ H4 H3).
    destruct_hypos;subst...
Qed.

Lemma binds_uniq': forall X U T E1 E2,
  wf_env (E1 ++ [(X, bind_sub T)] ++ E2) ->
  binds X (bind_sub U) (E1 ++ [(X, bind_sub T)] ++ E2) ->
  U = T.
Proof.
  intros.
  assert (binds X (bind_sub T) (E1 ++ [(X, bind_sub T)] ++ E2)) by auto.
  pose proof binds_uniq _ _ _ H H0 H1... inversion H2.
  auto.
Qed.

Lemma sub_tvar_chain_sub_env: forall E W X Y,
  sub_tvar_chain E W X Y -> 
  sub_env E W.
Proof with auto.
  intros. dependent induction H;intros...
  - apply binds_split in H0. destruct H0 as [E1 [E2 H0]].
    rewrite H0. apply sub_env_app.
    apply sub_env_cons_some. apply sub_env_nil.
  - pose proof WF_from_binds_typ_strong _ _ _ _ H.
    inversion H1;subst.
    + apply binds_split in H4.
      destruct H4 as [E3 [E4 H5]]. subst E2.
      apply sub_env_app.
      apply sub_env_cons_some.
      apply sub_env_app.
      inversion H0;subst.
      { rewrite <- app_assoc in H3.
        rewrite <- app_assoc in H3. apply binds_uniq' in H3.
        2:{ rewrite !app_assoc... }
        subst U0. apply sub_env_cons_some. apply sub_env_nil. }
      { rewrite <- app_assoc in IHsub_tvar_chain.
        rewrite <- app_assoc in IHsub_tvar_chain.
        replace (E1 ++ (X1, bind_sub X2) :: E3 ++ (X2, bind_sub U) :: E4)
        with ((E1 ++ (X1, bind_sub X2) :: E3) ++ [(X2, bind_sub U)] ++ E4) in H2.
        2:{ rewrite !app_assoc... }
        replace (E0 ++ (X2, bind_sub X4) :: E2) with
        (E0 ++ [(X2, bind_sub X4)] ++ E2) in H2...
        pose proof uniq_from_wf_env H3.
        pose proof uniq_split_eq _ _ _ _ _ _ _ H5 H2.
        destruct_hypos. inversion H7. subst.
        apply sub_env_cons_some.
        apply sub_env_cut_heads in IHsub_tvar_chain...
        apply uniq_from_wf_env... rewrite !app_assoc...
      }
    +
      inversion H0;subst.
      * assert (binds X3 (bind_sub_lb U) (E1 ++ X1 ~ bind_sub X3 ++ E2))...
        pose proof binds_uniq _ _ _ H2 H3 H5.
        inversion H6.
      * assert (binds X2 (bind_sub X4) (E1 ++ X1 ~ bind_sub X2 ++ E2))...
        { simpl in H2. simpl. rewrite <- H2... }
        assert (binds X2 (bind_sub_lb U) (E1 ++ X1 ~ bind_sub X2 ++ E2))...
        simpl in H3. rewrite H2 in H3.
        pose proof binds_uniq _ _ _ H3 H6 H7.
        inversion H8.      
Qed.

Lemma sub_tvar_chain_tail_in: forall E W X Y,
  sub_tvar_chain E W X Y ->
  exists W' T, W = W' ++ Y ~ bind_sub T.
Proof with auto.
  intros.
  induction H...
  - exists nil, ( U)...
  - destruct IHsub_tvar_chain as [W' [T IH]].
    exists (X1 ~ bind_sub X2 ++ W'), T.
    rewrite IH...
Qed.

Lemma sub_env_binds: forall E W X T,
  sub_env E W ->
  binds X T W ->
  binds X T E.
Proof with auto.
  intros.
  induction H...
  - inversion H0.
  - destruct H0... inversion H0...
Qed.

Lemma sub_tvar_chain_antisym: forall E W1 W2 X Y,
  sub_tvar_chain E W1 X Y -> 
  sub_tvar_chain E W2 Y X -> 
  X = Y.
Proof with auto.
  intros.
  inversion H;subst...
  inversion H0;subst...
  destruct (X == Y)...

  assert (exists T, binds Y (bind_sub T) E2).
  {
    pose proof sub_tvar_chain_sub_env H.
    apply sub_env_cut_heads in H6.
    2:{ apply uniq_from_wf_env... }
    pose proof sub_tvar_chain_tail_in H.
    destruct H7 as [W' [T H7]].
    destruct W';inversion H7;subst...
    { exfalso... }
    exists T.
    apply sub_env_binds with (X:=Y) (T:=bind_sub T) in H6...
  }

  assert (exists T, binds X (bind_sub T) E3).
  {
    rewrite !cons_app_one in H3. rewrite <- H3 in H0.
    pose proof sub_tvar_chain_sub_env H0.
    apply sub_env_cut_heads in H7.
    2:{ apply uniq_from_wf_env... }
    pose proof sub_tvar_chain_tail_in H0.
    destruct H8 as [W' [T H8]].
    destruct W';inversion H8;subst...
    { exfalso... }
    exists T. 
    apply sub_env_binds with (X:=X) (T:=bind_sub T) in H7...
  }

  destruct H6. destruct H7.
  apply binds_split in H6.
  apply binds_split in H7.
  destruct H6 as [E1a [E1b H6]], H7 as [E2a [E2b H7]].
  rewrite H6, H7 in H3.
  rewrite !cons_app_one in H3. rewrite <-app_assoc in H3.
  rewrite <- app_assoc in H3.
  apply uniq_split_eq in H3.
  2:{ apply uniq_from_wf_env. rewrite !app_assoc.
      rewrite H7 in H4... }
  destruct_hypos. subst.
  apply uniq_from_wf_env in H1.
  rewrite !app_assoc in H1.
  apply uniq_app_2 in H1.
  apply uniq_cons_2 in H1.
  rewrite !dom_app in H1...
  exfalso. apply H1. apply union_iff. right.
  apply union_iff. right. apply union_iff. right.
  apply union_iff. left. apply KeySetFacts.add_iff...
Qed.

(* Dual proof for lower bound *)

Lemma WF_from_binds_typ_strong_lb: 
forall (x : atom) (U : typ) (E1 E2 : env),
  wf_env (E1 ++ [(x, bind_sub_lb U)] ++ E2) -> 
  WF E2 U.
Proof with auto.
intros.
dependent induction H.
-
  destruct E1;inversion x.
-
  destruct E1;inversion x;subst...
  specialize (IHwf_env _ _ _ _ JMeq_refl)...
-
  destruct E1;inversion x;subst...
  specialize (IHwf_env _ _ _ _ JMeq_refl)...
-
  destruct E1;inversion x;subst...
  specialize (IHwf_env _ _ _ _ JMeq_refl)...
Qed.


Lemma binds_uniq_lb': forall X U T E1 E2,
  wf_env (E1 ++ [(X, bind_sub_lb T)] ++ E2) ->
  binds X (bind_sub_lb U) (E1 ++ [(X, bind_sub_lb T)] ++ E2) ->
  U = T.
Proof.
  intros.
  assert (binds X (bind_sub_lb T) (E1 ++ [(X, bind_sub_lb T)] ++ E2)) by auto.
  pose proof binds_uniq _ _ _ H H0 H1... inversion H2.
  auto.
Qed.

Lemma sub_tvar_chain_sub_env_lb: forall E W X Y,
  sub_tvar_chain_lb E W X Y -> 
  sub_env E W.
Proof with auto.
  intros. dependent induction H;intros...
  - apply binds_split_lb in H0. destruct H0 as [E1 [E2 H0]].
    rewrite H0. apply sub_env_app.
    apply sub_env_cons_some. apply sub_env_nil.
  - pose proof WF_from_binds_typ_strong_lb _ _ _ _ H.
    inversion H1;subst.
    +
      inversion H0;subst.
      * assert (binds X2 (bind_sub U)  (E1 ++ X3 ~ bind_sub_lb X2 ++ E2))...
        pose proof binds_uniq _ _ _ H2 H3 H5.
        inversion H6.
      * assert (binds X2 (bind_sub_lb X4) (E1 ++ X3 ~ bind_sub_lb X2 ++ E2))...
        { simpl in H2. simpl. rewrite <- H2... }
        assert (binds X2 (bind_sub U) (E1 ++ X3 ~ bind_sub_lb X2 ++ E2))...
        simpl in H3. rewrite H2 in H3.
        pose proof binds_uniq _ _ _ H3 H6 H7.
        inversion H8.    
    + apply binds_split_lb in H4.
      destruct H4 as [E3 [E4 H5]]. subst E2.
      apply sub_env_app.
      apply sub_env_cons_some.
      apply sub_env_app.
      inversion H0;subst.
      { rewrite <- app_assoc in H3.
        rewrite <- app_assoc in H3. apply binds_uniq_lb' in H3.
        2:{ rewrite !app_assoc... }
        subst U0. apply sub_env_cons_some. apply sub_env_nil. }
      { rewrite <- app_assoc in IHsub_tvar_chain_lb.
        rewrite <- app_assoc in IHsub_tvar_chain_lb.
        replace (E1 ++ (X3, bind_sub_lb X2) :: E3 ++ (X2, bind_sub_lb U) :: E4)
        with ((E1 ++ (X3, bind_sub_lb X2) :: E3) ++ [(X2, bind_sub_lb U)] ++ E4) in H2.
        2:{ rewrite !app_assoc... }
        replace (E0 ++ (X2, bind_sub_lb X4) :: E2) with
        (E0 ++ [(X2, bind_sub_lb X4)] ++ E2) in H2...
        pose proof uniq_from_wf_env H3.
        pose proof uniq_split_eq _ _ _ _ _ _ _ H5 H2.
        destruct_hypos. inversion H7. subst.
        apply sub_env_cons_some.
        apply sub_env_cut_heads in IHsub_tvar_chain_lb...
        apply uniq_from_wf_env... rewrite !app_assoc...
      }  
Qed.

Lemma sub_tvar_chain_tail_in_lb: forall E W X Y,
  sub_tvar_chain_lb E W X Y ->
  exists W' T, W = W' ++ X ~ bind_sub_lb T.
Proof with auto.
  intros.
  induction H...
  - exists nil, ( U)...
  - destruct IHsub_tvar_chain_lb as [W' [T IH]].
    exists (X3 ~ bind_sub_lb X2 ++ W'), T.
    rewrite IH...
Qed.


Lemma sub_tvar_chain_antisym_lb: forall E W1 W2 X Y,
  sub_tvar_chain_lb E W1 X Y -> 
  sub_tvar_chain_lb E W2 Y X -> 
  X = Y.
Proof with auto.
  intros.
  inversion H;subst...
  inversion H0;subst...
  destruct (X == Y)...


  assert (exists T, binds X (bind_sub_lb T) E2).
  {
    pose proof sub_tvar_chain_sub_env_lb H.
    apply sub_env_cut_heads in H6.
    2:{ apply uniq_from_wf_env... }
    pose proof sub_tvar_chain_tail_in_lb H.
    destruct H7 as [W' [T H8]].
    destruct W';inversion H8;subst...
    { exfalso... }
    exists T.
    apply sub_env_binds with (X:=X) (T:=bind_sub_lb T) in H6...
  }

  assert (exists T, binds Y (bind_sub_lb T) E3).
  {
    rewrite !cons_app_one in H3. rewrite <- H3 in H0.
    pose proof sub_tvar_chain_sub_env_lb H0.
    apply sub_env_cut_heads in H7.
    2:{ apply uniq_from_wf_env... }
    pose proof sub_tvar_chain_tail_in_lb H0.
    destruct H8 as [W' [T H8]].
    destruct W';inversion H8;subst...
    { exfalso... }
    exists T. 
    apply sub_env_binds with (X:=Y) (T:=bind_sub_lb T) in H7...
  }

  
  destruct H6. destruct H7.
  apply binds_split_lb in H6.
  apply binds_split_lb in H7.
  destruct H6 as [E1a [E1b H6]], H7 as [E2a [E2b H7]].
  rewrite H6, H7 in H3.
  rewrite !cons_app_one in H3. rewrite <-app_assoc in H3.
  rewrite <- app_assoc in H3.
  apply uniq_split_eq in H3.
  2:{ apply uniq_from_wf_env. rewrite !app_assoc.
      rewrite H7 in H4... }
  destruct_hypos. subst.
  apply uniq_from_wf_env in H1.
  rewrite !app_assoc in H1.
  apply uniq_app_2 in H1.
  apply uniq_cons_2 in H1.
  rewrite !dom_app in H1...
  exfalso. apply H1. apply union_iff. right.
  apply union_iff. right. apply union_iff. right.
  apply union_iff. left. apply KeySetFacts.add_iff...
Qed.