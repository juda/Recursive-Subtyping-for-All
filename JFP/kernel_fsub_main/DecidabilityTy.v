...
Proof with auto.
  ...
  - induction T2;simpl in H1; try solve [inversion H1]...
    destruct (n == n0);inversion H1. simpl in H, H0.
    inv H1. apply IHT1_1 in H4...
    apply IHT1_2 in H5... subst...
Qed.
...
Proof with auto.
  ...
  -
    ...
    { pose proof IHHty1 _ H3. subst.
      epose proof exposure_uniq _ H5 H.
      Unshelve. 2:{ apply Algo.typing_regular in Hty2. destruct_hypos... }
      subst. rewrite H0 in H7. inv H7...
    }  
  -
    inversion Hty2;subst... f_equal.
    + f_equal.
...
Lemma exposure_any_dec_aux: forall k E A,
  List.length E <= k ->
  wf_env E -> WF E A ->
  { A1 | Algo.exposure E A (A1) }.
Proof with auto.
  intros k. induction k;intros.
  {
    destruct A; 
    ...
    - exfalso. inv H1.
    - destruct E; simpl in H; try lia.
      exfalso. inv H1. inv H4.
    - exists (typ_arrow A1 A2)...
    - exists (typ_all A1 A2)...
    - exists (typ_mu A)...
    - exists (typ_label a A)...
    - exists (typ_rcd_nil)...
    - exists (typ_rcd_cons a A1 A2)...
  }
  destruct A; simpl in H0; try solve [right; intros A1' C; inversion C].
  - exists typ_nat;simpl...
  - exists typ_top;simpl...
...
Lemma exposure_any_dec: forall E A,
  wf_env E -> WF E A ->
  { A1 | Algo.exposure E A (A1) }.
Proof with auto.
  intros. apply exposure_any_dec_aux with (k:=List.length E)...
Qed.
...
Proof with auto.
  destruct A; 
  simpl in *; try solve [right; intros A1' C; inversion C].
  - left. exists typ_top. constructor...
  - left. exists A. constructor...
Qed.
...
Proof with auto.
  ...
  - 
    ...
    destruct (exposure_tabs_dec w H0).
    {
      destruct s as [A1 [A2 ?]].
    ...
    { destruct s as [A' ?].
      ...
      + right. intros A'' C. inv C.
        * pose proof exposure_uniq2 w H6 e0.
          pose proof algotyping_uniq t0 H3.
          subst. inv H1. apply n...
    }
    { right. intros A' C. inv C.
      * apply n in H6... }
  -
    left. exists typ_rcd_nil...
  -
    (* cons *)
    ...
    destruct (KeySetProperties.In_dec a ((Algo.collectLabele e2))).
    {
      right. intros A' C. inv C. fsetdec.
    }
    destruct s as [T1 ?], s0 as [T2 ?].
    destruct (rt_type_dec T2).
    2:{
      right. intros A' C. inv C. apply n0...
      pose proof algotyping_uniq t0 H4. subst...
    }
    left. exists (typ_rcd_cons a T1 T2)...
  -
    (* proj *)
    destruct (IHk E e). { lia. }
    2:{
    ...
    destruct (exposure_any_dec w H0) as [A'' ?].
    destruct (Tlookup a A'') eqn:Elookup.
    { left. exists t0. eapply Algo.typing_proj;eassumption. }
    { right. intros A C. inv C.
      pose proof algotyping_uniq t H3. subst.
      pose proof exposure_uniq w e0 H5. subst.
      rewrite H7 in Elookup. inv Elookup. }
Qed.
